...
if [[ -z "$FLAG_MACHINE" ]]; then
	if [[ "$DETECTED_IS_PI" == 1 ]]; then
		FLAG_MACHINE="pi"
	elif [[ "$DETECTED_EPHEMERAL" == 1 ]]; then
		FLAG_MACHINE="ephemeral"
	fi
fi
...
[[ -x "$GUM_BIN" ]] || {
	err "Gum binary not executable: $GUM_BIN"
	exit 1
}
...
prompt_required() {
	...
	if [[ -z "${!var}" ]]; then
		if [[ "$FLAG_NON_INTERACTIVE" == 1 ]]; then
			err "Required: $var (use the matching CLI flag or DOTFILES_* env var)"
			exit 1
		fi
		if [[ -n "$choices" ]]; then
...
install_chezmoi_direct() {
	...
	case "$DETECTED_OS" in
		...
		*)
			err "chezmoi install: unsupported OS: $DETECTED_OS"
			return 1
			;;
	esac
	case "$DETECTED_ARCH" in
		...
		*)
			err "chezmoi install: unsupported arch: $DETECTED_ARCH"
			return 1
			;;
	esac
