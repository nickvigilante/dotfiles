...
`~/.env` is generated by chezmoi from a template (`home/private_dot_env.tmpl`) and sourced by `.zshenv`. The template pulls values from an external secret store — never from the repo itself. Supported backends: [1Password](https://www.chezmoi.io/user-guide/secrets/1password/), [Bitwarden](https://www.chezmoi.io/user-guide/secrets/bitwarden/), [age encryption](https://www.chezmoi.io/user-guide/encryption/age/), or the OS keychain.

See [chezmoi's secrets documentation](https://www.chezmoi.io/user-guide/secrets/) to configure whichever backend you use.
